...
function setupGameSelector() {
    ...
    gameSelect.addEventListener('change', (e) => {
        ...
        if (selectedGame) {
            switchTab(selectedGame);
        } else {
            // Hide all tab contents if no game selected
...
async function loadChessStatistics() {
    const chessStatsDiv = document.getElementById('chessStats');
    
    try {
        const { data: scores, error: scoresError } = await supabase
            .from('chess_scores')
            .select('user_uid, opponent_uid, is_computer_opponent, result, moves_count, game_duration_seconds, created_at')
            .order('created_at', { ascending: false });
        
        if (scoresError) throw scoresError;
        
        if (!scores || scores.length === 0) {
            chessStatsDiv.innerHTML = '<div class="no-data">No Chess game data available yet. Play the game to see statistics!</div>';
            return;
        }
        
        const userIds = [...new Set(scores.map(s => s.user_uid).concat(scores.map(s => s.opponent_uid).filter(Boolean)))];
        const { data: users, error: usersError } = await supabase
            .from('Users')
            .select('UID, First_Name, Last_Name, Username')
            .in('UID', userIds);
        ...
        scores.forEach(score => {
            ...
            else if (score.result === 'draw') userStats[score.user_uid].draws++;
            
            userStats[score.user_uid].totalMoves += Number(score.moves_count || 0);
            userStats[score.user_uid].totalTime += Number(score.game_duration_seconds || 0);
            
            if (score.is_computer_opponent) {
                userStats[score.user_uid].gamesVsComputer++;
            } else {
                userStats[score.user_uid].gamesVsHuman++;
            }
        });
        ...
        chessStatsDiv.innerHTML = '';
        chessStatsDiv.appendChild(table);
...
async function loadCheckersStatistics() {
    const checkersStatsDiv = document.getElementById('checkersStats');
    ...
    try {
        ...
        if (!scores || scores.length === 0) {
            checkersStatsDiv.innerHTML = '<div class="no-data">No Checkers game data available yet. Play the game to see statistics!</div>';
            return;
        }
        ...
        checkersStatsDiv.innerHTML = '';
        checkersStatsDiv.appendChild(table);
        ...
        const isMissingTable = /table.*does not exist/i.test(msg) || 
                              ...
                              error?.code === '42P01' ||
                              error?.code === 'PGRST116';
        checkersStatsDiv.innerHTML = isMissingTable
            ? '<div class="no-data">Checkers statistics table is not set up yet.<br><small>Run <code>create_checkers_table.sql</code> in Supabase, then play a game.</small></div>'
            : `<div class="no-data">Error loading Checkers statistics: ${msg}</div>`;
    }
}
